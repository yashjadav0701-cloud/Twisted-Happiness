...
const CATALOG_REFRESH_SEED = `${Date.now()}-${Math.random()}`;
  ...
  async function initialiseApp() {
    try {
      ...
      // Delay push prompt by 0.8 seconds so it doesn't interrupt their first impression
      setTimeout(initCustomerPushNotifications, 800);
    } catch (error) {
  ...
  function normaliseCartItem(item) {
    ...
    return {
      ...
      image: Utils.safeImageURL(item.image || item.thumbImg || '', '/assets/th_logo.svg?v=mu6p10br'),
  ...
  async function initCustomerPushNotifications() {
    ...
    try {
      ...
      // 🔥 FIX 1: If they granted permission previously but the browser dropped the token, silently restore it!
      if (Notification.permission === 'granted') {
        await subscribeCustomerToPush(true); // true = silent background execution
        return;
      }

      // 🔥 FIX 2: Ask again on EVERY refresh if they haven't decided yet
  ...
  // --- Premium Customer Push Engine ---
  async function subscribeCustomerToPush(isSilent = false) {
    const btn = document.getElementById('push-optin-yes');
    if (btn && !isSilent) { btn.textContent = 'Enabling...'; btn.disabled = true; }
    ...
    try {
      ...
      // 🔥 NEW SECURE VAPID KEY INJECTED
      const publicVapidKey = 'BJ6SjbC7FtweHxs9qz2VaUcJCpn_3I8rG8kgFDu5YO7pZjRr_glDRjrrcG04xtmbnDPeZAC9MvwCre_MenpoVgM'; 
      const applicationServerKey = urlBase64ToUint8Array(publicVapidKey);

      // 1. NUCLEAR RESET: Force Chrome to mathematically destroy any ghost Service Workers
      const registrations = await navigator.serviceWorker.getRegistrations();
      for (let reg of registrations) {
        await reg.unregister();
      }

      // 2. Freshly register the Service Worker
      const registration = await navigator.serviceWorker.register('/sw.js');
      await navigator.serviceWorker.ready;
      await new Promise(r => setTimeout(r, 600)); // Brief pause for OS to catch up

      // 3. Attempt the clean subscription
      ...
      await supabaseClient.from('customer_push_subscriptions').delete().eq('endpoint', subJSON.endpoint);
      await supabaseClient.from('customer_push_subscriptions').upsert({
        device_id: getOrCreateDeviceId(),
      ...
      if (!isSilent) {
        ...
        try {
          await registration.showNotification('✨ Welcome to the VIP List!', {
            body: "You'll now be the first to know about new handcrafted drops, exclusive discounts, and studio updates. 🌸",
          ...
          console.warn('Welcome push failed to render locally:', pushErr);
        }
      }

    } catch (err) {
      // 4. If it STILL fails, print the exact Android OS error to the screen so we know why
      console.error("Push Subscription Failed:", err);
      document.getElementById('push-optin-overlay')?.classList.add('hidden');
      
      await Utils.choice({ 
        title: 'Subscription Failed', 
        message: `Chrome OS Error: "${err.message}". \n\nPlease ensure your device settings allow notifications for Chrome.`, 
        icon: '⚠️', 
        hideSecondary: true,
        primaryLabel: 'Okay' 
      });
    } finally {
      if (btn && !isSilent) { btn.textContent = 'Yes, notify me'; btn.disabled = false; }
    }
  }
  ...
  function applyStoreConfiguration() {
    ...
    const defaultImg = '/assets/th_logo.svg?v=mu6p10br';
  ...
  function initialiseCatalog() {
    ...
    if (!catalogEventsBound) {
      ...
      function closeSearch(fromPopState = false) {
        if (!searchOverlay || !searchOverlay.classList.contains('is-active')) return; // FIX: Prevents double-firing history.back()
  ...
  function renderSearchSuggestions() {
    ...
    host.innerHTML = results.map((result, index) => `
      <button class="search-suggestion ${index === state.searchSuggestionIndex ? 'is-active' : ''}" type="button" role="option" aria-selected="${index === state.searchSuggestionIndex ? 'true' : 'false'}" data-search-product="${Utils.escapeHTML(result.product.id)}">
        <img src="${Utils.escapeHTML(result.product.images?.[0] || '/assets/th_logo.svg?v=mu6p10br')}" alt="" loading="lazy" decoding="async">
        <span>
  ...
  function renderHeavyCategories() {
    ...
    mainCategories.forEach(cat => {
      ...
      const subCatHtml = subCategories.slice(0, 6).map(sub => {
        ...
        const thumb = subProduct ? subProduct.images[0] : '/assets/th_logo.svg?v=mu6p10br';
        return `
    ...
    return `<article class="product-card ${revealedClass} ${layoutClass}" data-product-id="${Utils.escapeHTML(product.id)}">
      <div class="product-card__image-container">
        <a class="product-card__image" href="${Utils.escapeHTML(productURL(product))}" aria-label="View ${Utils.escapeHTML(product.title)}">
          <img src="${Utils.escapeHTML(product.images[0] || '/assets/th_logo.svg?v=mu6p10br')}" alt="${Utils.escapeHTML(product.title)}" loading="lazy" decoding="async">
        ...
        <button type="button" class="quick-add-btn pop-click" data-card-action="${isCanvas ? 'choose' : 'add'}" aria-label="${isCanvas ? 'Choose size' : 'Quick add'}">
          ${isCanvas 
            ? '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14"/><path d="m19 12-7 7-7-7"/></svg>' 
            : '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/><path d="M12 10v4"/><path d="M10 12h4"/></svg>'}
        </button>
      </div>
  ...
  function renderProduct(product) {
    ...
    const primaryImage =
      product.images?.[0] ||
      `${window.location.origin}/assets/share-icon.png?v=mu6p10br`;
  ...
  function renderGallery(product) {
    const images = product.images.length ? product.images : ['/assets/th_logo.svg?v=mu6p10br'];
  ...
  function buildCartItem(product, selections = {}) {
    ...
    return {
      ...
      title: product.title,
      image: product.images?.[0] || '/assets/th_logo.svg?v=mu6p10br',
  ...
  function cartItemMarkup(item, location) {
    ...
    return `<article class="${location === 'checkout' ? 'checkout-item' : 'cart-item'}" data-cart-key="${Utils.escapeHTML(item.key)}" style="display: flex; gap: 12px; align-items: center; position: relative;">
      <img src="${Utils.escapeHTML(item.image || '/assets/th_logo.svg?v=mu6p10br')}" alt="" style="width: 56px; height: 56px; object-fit: cover; border-radius: 6px; flex-shrink: 0; border: 1px solid var(--line);">
  ...
  function checkoutCompactItemMarkup(item) {
    return `<div style="display: flex; gap: 12px; margin-bottom: 16px; align-items: start;">
      <img src="${Utils.escapeHTML(item.image || '/assets/th_logo.svg?v=mu6p10br')}" alt="" style="width: 44px; height: 44px; object-fit: cover; border-radius: var(--radius-sm); background: var(--beige); flex-shrink: 0; border: 1px solid var(--line);">

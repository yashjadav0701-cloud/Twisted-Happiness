...
const CATALOG_REFRESH_SEED = `${Date.now()}-${Math.random()}`;
  ...
  function normaliseCartItem(item) {
    ...
    return {
      ...
      image: Utils.safeImageURL(item.image || item.thumbImg || '', '/assets/th_logo.svg?v=mtbne5lx'),
  ...
  function renderSearchSuggestions() {
    ...
    host.innerHTML = results.map((result, index) => `
      <button class="search-suggestion ${index === state.searchSuggestionIndex ? 'is-active' : ''}" type="button" role="option" aria-selected="${index === state.searchSuggestionIndex ? 'true' : 'false'}" data-search-product="${Utils.escapeHTML(result.product.id)}">
        <img src="${Utils.escapeHTML(result.product.images?.[0] || '/assets/th_logo.svg?v=mtbne5lx')}" alt="" loading="lazy" decoding="async">
        <span>
  ...
  function productCard(product, compact = false) {
    ...
    return `<article class="product-card ${revealedClass}" data-product-id="${Utils.escapeHTML(product.id)}">
      <a class="product-card__image" href="${Utils.escapeHTML(productURL(product))}" aria-label="View ${Utils.escapeHTML(product.title)}">
        <img src="${Utils.escapeHTML(product.images[0] || '/assets/th_logo.svg?v=mtbne5lx')}" alt="${Utils.escapeHTML(product.title)}" loading="lazy" decoding="async">
  ...
  function productShareURL(product) {
    return `https://twistedhappiness.vercel.app//share?pid=${encodeURIComponent(product.id)}`;
  }
  ...
  function renderProduct(product) {
    ...
    const primaryImage =
      product.images?.[0] ||
      `${window.location.origin}/assets/share-icon.png?v=mtbne5lx`;
  ...
  function renderGallery(product) {
    const images = product.images.length ? product.images : ['/assets/th_logo.svg?v=mtbne5lx'];
  ...
  function buildCartItem(product, selections = {}) {
    ...
    return {
      ...
      title: product.title,
      image: product.images?.[0] || '/assets/th_logo.svg?v=mtbne5lx',
  ...
  function cartItemMarkup(item, location) {
    ...
    return `<article class="${location === 'checkout' ? 'checkout-item' : 'cart-item'}" data-cart-key="${Utils.escapeHTML(item.key)}">
      <img src="${Utils.escapeHTML(item.image || '/assets/th_logo.svg?v=mtbne5lx')}" alt="${Utils.escapeHTML(item.title)}">
      <div>
  ...
  function checkoutCompactItemMarkup(item) {
    return `<div style="display: flex; gap: 12px; margin-bottom: 16px; align-items: start;">
      <img src="${Utils.escapeHTML(item.image || '/assets/th_logo.svg?v=mtbne5lx')}" alt="" style="width: 44px; height: 44px; object-fit: cover; border-radius: var(--radius-sm); background: var(--beige); flex-shrink: 0; border: 1px solid var(--line);">
